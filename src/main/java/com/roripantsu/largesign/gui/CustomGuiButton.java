...
public class CustomGuiButton extends GuiButton {
	...
	private CustomGuiButton.buttonStyle style;

	public CustomGuiButton(int ID, int X, int Y, int W, int H,
			CustomGuiButton.buttonStyle style, Object obj) {
		super(ID, X, Y, W, H, "");
		this.style = style;

		if (style == CustomGuiButton.buttonStyle.ITEMICON)
			this.itemStack = (ItemStack) obj;

		if (style == CustomGuiButton.buttonStyle.COLORBLOCK)
	...
	private void drawColorBlockButton(Minecraft MC, int X, int Y) {
		...
		this.renderQuad(this.xPosition, this.yPosition, this.xPosition
				+ this.width, this.yPosition + this.height, new Color(225, 225,
				225, 32 * k));
